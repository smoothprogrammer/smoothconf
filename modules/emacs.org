...
#+begin_src elisp :noweb yes
  (use-package ryo-modal
    :hook (after-init . global-ryo-modal-mode)
    :bind <<insert-mode>>
    ...
    (ryo-modal-keys
     ("a" forward-char :first '(smooth-deactivate-mark) :exit t)
     ("A" move-end-of-line :first '(smooth-deactivate-mark) :exit t)
     ("b" backward-word :first '(smooth-activate-mark))
     ("B" backward-word :first '(smooth-activate-mark-if-inactive))
     ("c" smooth-delete :exit t)
     ("C" kill-line :exit t)
     ("d" smooth-delete)
     ("D" kill-line)
     ;; ("e" :name "flymake")
     ("E" undefined)
     ("f" forward-word :first '(smooth-activate-mark))
     ("F" forward-word :first '(smooth-activate-mark-if-inactive))
     ;; ("g" :name "goto")
     ("G" undefined)
     ("h" backward-char :first '(smooth-deactivate-mark))
     ("H" backward-char :first '(smooth-activate-mark-if-inactive))
     ("i" ignore :first '(smooth-deactivate-mark) :exit t)
     ("I" move-beginning-of-line :first '(smooth-deactivate-mark) :exit t)
     ("j" next-line :first '(smooth-deactivate-mark))
     ("J" next-line :first '(smooth-activate-mark-if-inactive))
     ("k" previous-line :first '(smooth-deactivate-mark))
     ("K" previous-line :first '(smooth-activate-mark-if-inactive))
     ("l" forward-char :first '(smooth-deactivate-mark))
     ("L" forward-char :first '(smooth-activate-mark-if-inactive))
     ("m" undefined)
     ("M" undefined)
     ("n" undefined)
     ("N" undefined)
     ("o" smooth-open-line-below :first '(smooth-deactivate-mark) :exit t)
     ("O" smooth-open-line-above :first '(smooth-deactivate-mark) :exit t)
     ("p" yank)
     ("P" undefined)
     ("q" quit-window)
     ("Q" undefined)
...
#+begin_src elisp :tangle no :noweb-ref smooth-config
  ...
  (defun smooth-activate-mark-if-inactive ()
    (unless (use-region-p) (set-mark (point))))

  (defun smooth-deactivate-mark ()
    (deactivate-mark))

  (defun smooth-delete (count)
    (interactive "p")
    (if (use-region-p)
  	  (kill-region (region-beginning) (region-end))
    (delete-char count t)))

  (defun smooth-open-line-below (count)
    (interactive "p")
    (end-of-line)
    (open-line count)
    (forward-line count))

  (defun smooth-open-line-above (count)
    (interactive "p")
    (beginning-of-line)
    (open-line count))
  ...
  (defun smooth-line (count)
    (interactive "p")
    (beginning-of-line)
    (smooth-activate-mark-if-inactive)
    (forward-line count))
#+end_src

* Movement and selection :h:j:k:l:f:g:v:w:semicolon:
...
#+begin_src elisp :noweb yes
  (use-package consult
    :ryo
    ("g m" consult-imenu :name "goto menu")
    ("g e" consult-flymake :name "goto error list")
    :custom
...
#+begin_src elisp :noweb yes
  (use-package eglot
    :ryo
    ("r" eglot-rename)
    ("g i" eglot-find-implementation :name "goto implementation")
    :hook
...
#+begin_src elisp
  (use-package xref
    :ryo
    ("g d" xref-find-definitions :name "goto definitions")
    ("g r" xref-find-references :name "goto references"))
#+end_src
