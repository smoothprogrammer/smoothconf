...
#+begin_src elisp :tangle no :noweb-ref smooth-config
  ...
  (defvar smooth--extend-p nil)

  (defun smooth-toggle-extend ()
    (interactive)
    (setq smooth--extend-p (not smooth--extend-p))
    (if smooth--extend-p
  	  (smooth-activate-mark)
  	(smooth-deactivate-mark)))

  (defun smooth-deactivate-extend ()
    (interactive)
    (setq smooth--extend-p nil)
    (smooth-deactivate-mark))
