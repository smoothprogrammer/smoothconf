...
#+begin_src elisp :tangle no :noweb yes :noweb-ref smooth-motion
    <<defun-smooth-motion>>
    ...
    (ryo-modal-keys
     ("H" smooth-line-left)
     ("J" smooth-paragraph-down)
     ("K" smooth-paragraph-up)
     ("L" smooth-line-right))
#+end_src

#+begin_src elisp :tangle no :noweb-ref defun-smooth-motion
  ...
  (defun smooth-left (count)
    (interactive "p")
    (unless smooth--anchor-p
  	(smooth-deactivate-mark))
    (backward-char count))

  (defun smooth-down (count)
    (interactive "p")
    (unless smooth--anchor-p
  	(smooth-deactivate-mark))
    (next-line count))

  (defun smooth-up (count)
    (interactive "p")
    (unless smooth--anchor-p
  	(smooth-deactivate-mark))
    (previous-line count))

  (defun smooth-right (count)
    (interactive "p")
    (unless smooth--anchor-p
  	(smooth-deactivate-mark))
    (forward-char count))

  (defun smooth-word-left (count)
    (interactive "p")
    (unless smooth--anchor-p
  	(smooth-activate-mark))
    (backward-word count))

  (defun smooth-word-right (count)
    (interactive "p")
    (unless smooth--anchor-p
  	(smooth-activate-mark))
    (forward-word count))

  (defun smooth-line-left (count)
    (interactive "p")
    (unless smooth--anchor-p
  	(smooth-activate-mark))
    (beginning-of-line count))

  (defun smooth-line-right (count)
    (interactive "p")
    (unless smooth--anchor-p
  	(smooth-activate-mark))
    (end-of-line count))

  (defun smooth-paragraph-down (count)
    (interactive "p")
    (unless smooth--anchor-p
  	(smooth-activate-mark))
    (forward-paragraph count))

  (defun smooth-paragraph-up (count)
    (interactive "p")
    (unless smooth--anchor-p
  	(smooth-activate-mark))
...
#+begin_src elisp :tangle no :noweb yes :noweb-ref smooth-insert
  <<defun-smooth-insert>>

  (ryo-modal-keys
   ("i" smooth-insert)
   ("a" smooth-append)
   ("o" smooth-open-line-below)
   ("c" smooth-change))

  (ryo-modal-keys
   ("I" smooth-insert-beginning)
   ("A" smooth-append-end)
   ("O" smooth-open-line-above))
#+end_src

#+begin_src elisp :tangle no :noweb-ref defun-smooth-insert
  (defun smooth-insert ()
    (interactive)
    (smooth-insert-mode))

  (defun smooth-insert-beginning ()
    (interactive)
    (beginning-of-line)
    (smooth-insert-mode))
...
#+begin_src elisp :tangle no :noweb yes :noweb-ref smooth-editing
  <<defun-smooth-editing>>
...
#+begin_src elisp :tangle no :noweb-ref defun-smooth-editing
  (defun smooth-undo ()
    (interactive)
    (undo-only)
    (smooth-deactivate-anchor))
  ...
  (defun smooth-put-menu (count)
    (interactive "p")
    (when (use-region-p)
  	(delete-region (region-beginning) (region-end)))
    (if (fboundp 'consult-yank-pop)
  	  (consult-yank-pop)
  	(yank-pop)))
...
#+begin_src elisp :tangle no :noweb yes :noweb-ref smooth-goto
  <<defun-smooth-goto>>

  (ryo-modal-keys
   ("g h" smooth-goto-left)
   ("g j" smooth-goto-down)
   ("g k" smooth-goto-up)
   ("g l" smooth-goto-right))

  (ryo-modal-keys
   ("g m" smooth-menu))
#+end_src

#+begin_src elisp :tangle no :noweb-ref defun-smooth-goto
  (defun smooth-goto-left ()
    (interactive)
    (unless smooth--anchor-p
  	(smooth-deactivate-mark))
    (beginning-of-line))

  (defun smooth-goto-down ()
    (interactive)
    (unless smooth--anchor-p
  	(smooth-deactivate-mark))
    (end-of-buffer))

  (defun smooth-goto-up ()
    (interactive)
    (unless smooth--anchor-p
  	(smooth-deactivate-mark))
    (beginning-of-buffer))

  (defun smooth-goto-right ()
    (interactive)
    (unless smooth--anchor-p
  	(smooth-deactivate-mark))
    (end-of-line))
