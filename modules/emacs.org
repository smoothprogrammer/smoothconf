...
#+begin_src elisp :tangle no :noweb-ref normal-mode
  ...
  ("o" ryo-open-line-below :exit t)
  ("O" ryo-open-line-above :exit t)
#+end_src
...
#+begin_src elisp :tangle no :noweb-ref ryo-helper
  ...
  (defun ryo-open-line-below ()
    (interactive)
    (end-of-line)
    (open-line 1)
    (forward-line 1))

  (defun ryo-open-line-above ()
    (interactive)
    (beginning-of-line)
    (open-line 1))
#+end_src

** Directory and file :SPC:d:f:
...
#+begin_src elisp :tangle no :noweb-ref jump
...
#+begin_src elisp :tangle no :noweb-ref help
  ("SPC h"
   (("m" describe-mode :name "describe mode")
    ("." helpful-at-point :name "describe at point")
