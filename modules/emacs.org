...
#+begin_src elisp :noweb yes
  ...
  (defun smooth--refresh-keymap ()
    (let ((entry (assq 'smooth-mode minor-mode-overriding-map-alist)))
      (cond
       ((eq smooth--state 'normal)
        (if entry
            (setcdr entry smooth-normal-map)
          (push `(smooth-mode . ,smooth-normal-map) minor-mode-overriding-map-alist)))
       ((eq smooth--state 'insert)
        (if entry
            (setcdr entry smooth-insert-map)
  ...
  (defmacro smooth-keys (&rest forms)
    (let ((keymap nil)
      ...
      (dolist (form forms)
        ...
        (let ((state (plist-get form :state)))
  		(cond
  		 (state
            (setq keymap (intern (format "smooth-%s-map" state))))
  		 (t
  		  (unless keymap
...
#+begin_src elisp :tangle no :noweb-ref smooth-motion
  ...
  ("g h" move-beginning-of-line :name line-begin :record t :select activate)
  ("g j" end-of-buffer :name buffer-end :record t :select activate)
  ("g k" beginning-of-buffer :name buffer-begin :record t :select activate)
  ("g l" move-end-of-line :name line-end :record t :select activate)
  ...
  ("G H" move-beginning-of-line :name extend-line-begin :select extend)
  ("G J" end-of-buffer :name extend-buffer-end :select extend)
  ("G K" beginning-of-buffer :name extend-buffer-begin :select extend)
  ("G L" move-end-of-line :name extend-line-end :select extend)
...
#+begin_src elisp :tangle no :noweb-ref org-insert
  ("SPC i t" org-insert-structure-template :name org-insert-template)
  ("SPC i h" org-babel-insert-header-arg :name org-insert-header)
  ("SPC i l" org-insert-link :name org-insert-link)
#+end_src
