...
#+begin_src elisp :tangle no :noweb-ref smooth-lsp
  ...
  (with-eval-after-load 'imenu
    (defun smooth-goto-menu ()
  	(interactive)
  	(if (fboundp 'consult-imenu)
  		(consult-imenu)
  	  (imenu)))
    (ryo-modal-keys
     ("g m" smooth-goto-menu :first '(smooth-deactivate-anchor))))

  (with-eval-after-load 'flymake
    (defun smooth-goto-error-menu ()
  	(interactive)
  	(if (fboundp 'consult-flymake)
  		(consult-flymake)
  	  (flymake-menu)))
    (ryo-modal-keys
     ("g e" smooth-goto-error-menu :first '(smooth-deactivate-anchor))))
#+end_src
