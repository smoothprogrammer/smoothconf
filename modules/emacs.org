...
- [X] flymake (builtin) → diagnostics (already works with Eglot)
- [X] consult-flymake → better diagnostics browsing
- [ ] tempel or yasnippet → snippets
- [X] marginalia → annotations in minibuffer
- [ ] embark → context-sensitive actions
- [X] corfu → completion frontend (already compatible)
- [X] orderless → fuzzy matching in Corfu/Consult
