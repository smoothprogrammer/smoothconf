...
#+begin_src elisp
  (defvar smooth-mode-map (make-sparse-keymap))

  (defun smooth--pulse (pulse)
    (when (memq pulse '(red green yellow))
      (let ((fn (intern (format "pulsar-pulse-line-%s" pulse))))
        (when (fboundp fn)
          (funcall fn)))))

  (defmacro smooth-keys (&rest bindings)
    `(progn
       ,@(mapcar
          (lambda (binding)
            (let* ((key (nth 0 binding))
              ...
              `(progn
                 ...
                 (keymap-set smooth-mode-map ,key ',name))))
          bindings)))

  (define-minor-mode smooth-mode
    "Toggle smooth modal editing.")

  (define-global-minor-mode global-smooth-mode smooth-mode
    (lambda () (smooth-mode 1)))

  (smooth-keys
   ("x" self-insert-command :name my-custom-command :pulse red)
   ("y" self-insert-command :name my-custom-command :pulse green))
#+end_src
